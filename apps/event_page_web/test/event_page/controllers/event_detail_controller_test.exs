defmodule EventPage.Web.EventDetailControllerTest do
  ...
  test "lists all entries on index", %{conn: conn} do
    ...
    assert html_response(conn, 200) =~ "Listing Event details"
  end

  test "renders form for new event_details", %{conn: conn} do
    conn = get conn, event_detail_path(conn, :new)
    assert html_response(conn, 200) =~ "New Event detail"
  end
  ...
  test "creates event_detail and redirects to show when data is valid", %{conn: conn} do
    ...
    assert redirected_to(conn) == event_detail_path(conn, :show, id)
  ...
  test "renders form for editing chosen event_detail", %{conn: conn} do
    ...
    conn = get conn, event_detail_path(conn, :edit, event_detail)
  ...
  test "updates chosen event_detail and redirects when data is valid", %{conn: conn} do
    ...
    assert redirected_to(conn) == event_detail_path(conn, :show, event_detail)

    conn = get conn, event_detail_path(conn, :show, event_detail)
  ...
  test "deletes chosen event_detail", %{conn: conn} do
    ...
    assert redirected_to(conn) == event_detail_path(conn, :index)
